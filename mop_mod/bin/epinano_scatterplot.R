...
input1 <- read.delim(zinput1, sep="," , stringsAsFactors = FALSE)  #1st variable
plotlab1 <- as.character(args[2])
plotlab2 <- as.character(args[4])
label1 <- gsub("-", "_", paste0("X", as.character(args[2])))  #1st label
input2 <- read.delim(zinput2,sep=",", stringsAsFactors = FALSE) #2nd variable
...
cleanup <- function(input, label) {
	...
	input$pos <- as.numeric(input$pos)
        ...
        #Convert frequencies into numeric data:
        input$mis <- as.numeric(input$mis)
        input$ins <- as.numeric(input$ins)
		...
		pdf(file=paste(chr,feature, plotlab1, plotlab2, "scatter.pdf", sep="_"),height=5,width=5,onefile=FALSE)
			print(ggplot(subs, aes_string(x=paste(label1, "value", sep="_"), y=paste(label2, "value", sep="_"))) +
				...
				geom_abline(slope=1, intercept=0,linetype="dashed")+
     				...
     				geom_text_repel(data=subset(subs, score>threshold), aes(label=Position), colour="black",segment.size  = 0.4,segment.color = "grey50",size=5)+
     				ggtitle(paste(feature, chr, sep="__"))+
				xlab(plotlab1)+
				ylab(plotlab2) +
				theme_bw()+
